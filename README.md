...
## 最近更新（来自 `index.html` v1.1）

- 版本号已升级为 **Ver. 1.1**，为班级专用版本（界面顶部显示为 Ver. 1.1）。
- 新增“考号认领”流程：用户在开始考试前必须在 `id-selection-screen` 页面中选择考号（01-48）。
- 新增考试确认/放弃的自定义模态框（`quit-confirm-modal`），替代浏览器 `confirm()`，并支持暂停/恢复考试（`isGamePaused`）。
- 殿试（难）模式增加了预备倒计时页（`countdown-screen`）和混合题库记忆键 `hard_mixed`。
- 答题区和计时器精度增强（计时以 0.01s 更新），超时与答错会触发红色闪烁效果与错题记录。
- 闪卡和复习模块增强：翻转动画、进度条及随机标题，闪卡播放页和选择页的交互优化。
- 设置增加“沉浸反馈/极速模式”切换（`setting-feedback-toggle`），用于控制答对效果的视觉展示。
- 添加“重置刷题记忆/清空榜单/销毁错题本”等高级数据管理按钮（设置页），并提供确认保护。
- 多处样式与交互优化（例如打印印章动画、排行榜标签页、便捷的科目按钮状态等）。

这些更改已经反映到应用前端（`index.html`）；若需要我把这些新功能逐条写入 `USER_GUIDE.md` 的使用说明或把变更记录写入单独 `CHANGELOG.md`，我可以继续执行。
