...
### 最近更新（index.html v1.1）

- 新增考号认领流程（`id-selection-screen`），用户需先认领考号后开始考试。
- 新增放弃确认模态框（`quit-confirm-modal`）与暂停逻辑（`isGamePaused`），替代浏览器 confirm。
- 殿试增加倒计时准备页与 `hard_mixed` 混合题库跟踪逻辑。
- 闪卡与复习模块交互优化（翻转动画、进度条、随机标题）。
- 设置项中添加答对反馈模式切换；增加数据管理快捷操作（重置记忆、清空榜单、销毁错题）。
